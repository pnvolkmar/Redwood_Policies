#
# Trans_MS_Coefficient.jl
#

using SmallModel

module Trans_MS_Coefficient
...
Base.@kwdef struct TControl
  ...
  PEM::VariableArray{4} = ReadDisk(db,"$CalDB/PEM")       # [Enduse,Tech,EC,Area] Process Efficiency ($/Btu)
  PEMBase::VariableArray{4} = ReadDisk(BCNameDB,"$CalDB/PEM")       # [Enduse,Tech,EC,Area] Process Efficiency ($/Btu)
...
function InputData(data::TControl)
  (; Areas,ECs) = data
  (; Enduses) = data
  (; Techs,Years) = data
  ...
  for enduse in Enduses, tech in Techs, ec in ECs, area in Areas, year in Years
    @finite_math PEE[enduse,tech,ec,area,year] = max(PEMBase[enduse,tech,ec,area]*PEMM[enduse,tech,ec,area,year]*
      (1/(1+(MCFUBase[enduse,tech,ec,area,year]/xInflation[area,year]*PEPM[enduse,tech,ec,area,year]/
...
function MarketShareNonPriceFactors(data::TControl)
  (; CalDB,db) = data
      ...
      if xMMSFMax > 0.0
        ...
        for tech in Techs
          @finite_math MAW[tech] = exp(log(MSMM[enduse,tech,ec,area,year])+
            ...
            log((MCFUBase[enduse,tech,ec,area,year]/xInflation[area,year]/PEE[enduse,tech,ec,area,year])/
        ...
        for tech in Techs
          if (MU[tech] != MUmax) && (MMSM0[enduse,tech,ec,area,year] >= 0.0)
...
function PolicyControl(db)
  @info"Trans_MS_Coefficient.jl - PolicyControl"
  TransPolicy(db)
end
