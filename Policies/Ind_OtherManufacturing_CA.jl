#
# Ind_OtherManufacturing_CA.jl - Other Industrial Manufacturing:
# 0% energy demand electrified by 2030 and 50% by 2045
#

using SmallModel

module Ind_OtherManufacturing_CA
...
function IndPolicy(db)
  ...
  # Construction
  #
  ecs = Select(EC,["Textiles","Lumber","Furniture","Fertilizer","Petroleum","Rubber",
  ...
  for ec in ecs, tech in techs, enduse in enduses
    DmFracMin[enduse,Electric,tech,ec,CA,Yr(2030)] = 
      max(DmFracMin[enduse,Electric,tech,ec,CA,Yr(2030)],0.0)
  end
...
function PolicyControl(db)
  @info "Ind_OtherManufacturing_CA.jl - PolicyControl"
  IndPolicy(db)
end
