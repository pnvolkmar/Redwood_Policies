...
function ComPolicy(db)
  ...
  for area in Areas, ec in ECs, enduse in Enduses, year in years
    xMMSFTotal[enduse,ec,area,year] = 
      sum(xMMSF[enduse,tech,ec,area,year] for tech in Techs)
  end
  
  for area in Areas, ec in ECs, tech in Techs, enduse in Enduses, year in years
    @finite_math xMMSF[enduse,tech,ec,area,year] = 
      xMMSF[enduse,tech,ec,area,year]/xMMSFTotal[enduse,ec,area,year]
  end
...
function PolicyControl(db)
  @info ("Com_MS_Normalize.jl - PolicyControl")
  ComPolicy(db)
end
