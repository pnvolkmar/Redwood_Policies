#
# Com_MS_Coefficient.jl
#

using SmallModel

module Com_MS_Coefficient
...
function InputData(data::CControl)
  (; Areas,ECs) = data
  (; Enduses) = data
  (; Techs,Years) = data
      ...
      if xMMSFMax > 0.0
        ...
        for tech in Techs
          @finite_math MAW[tech] = exp(log(MSMM[enduse,tech,ec,area,year])+
            MVF[enduse,tech,ec,area,year]*log((MCFUBase[enduse,tech,ec,area,year]/
              xInflation[area,year]/PEE[enduse,tech,ec,area,year])/
...
function PolicyControl(db)
  @info "Com_MS_Coefficient.jl - PolicyControl"
  ComPolicy(db)
end
