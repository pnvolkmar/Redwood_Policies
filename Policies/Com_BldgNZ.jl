#
# Com_BldgNZ.jl - Future building code standards
#
...
function ComPolicy(db)
  ...
  for year in years, area in areas
    EEImprovement[area,year] = 0.516
  end
  
  areas = Select(Area,"MB")
  years = collect(Yr(2030):Final)
  for year in years, area in areas
    EEImprovement[area,year] = 0.388
  end
  
  areas = Select(Area,"NB")
  years = collect(Yr(2030):Final)
  for year in years, area in areas
    EEImprovement[area,year] = 0.621
  end
  
  areas = Select(Area,"NS")
  years = collect(Yr(2030):Final)
  for year in years, area in areas
    EEImprovement[area,year] = 0.598
  end
  
  # TODO PROMULA: Is there a reason we write over QC values three times?
  areas = Select(Area,"QC")
  years = collect(Yr(2031):Final)
  for year in years, area in areas
    EEImprovement[area,year] = 0.415
  end
  years = collect(Yr(2031):Final)
  for year in years, area in areas
    EEImprovement[area,year] = 0.597
  end
  years = collect(Yr(2030):Final)
  for year in years, area in areas
    EEImprovement[area,year] = 0.597
  ...
  years = collect(Yr(2031):Final)
  for year in years, area in areas
    EEImprovement[area,year] = 0.799
  end
  
  areas = Select(Area,"PE")
  years = collect(Yr(2030):Final)
  for year in years, area in areas
    EEImprovement[area,year] = 0.489
  end
  
  areas = Select(Area,["NL","YT","NT","NU"])
  years = collect(Yr(2031):Final)
  for year in years, area in areas
    EEImprovement[area,year] = 0.302
  end
  ...
  areas = Select(Area,["ON","NB","NS","MB","SK","AB","PE","NL","NT","NU","YT"])
  years = collect(Yr(2030):Final)
...
function PolicyControl(db)
  @info "Com_BldgNZ.jl - PolicyControl"
  ComPolicy(db)
end
