...
function ElecPolicy(db)
  ...
  (; UnF1,UnFlFr,UnOnLine) = data
  (; UnPGratis,UnPlant,UnPolRef,UnSector,xETAPr) = data
  (; xExchangeRate,xExchangeRateNation,xFSell,xGoalPol,) = data
  (; xInflationNation,xISell,xPGratis,xPolCap) = data
  (; xUnGP) = data
  (; EIBase,OBA,OBAElectric) = data
  ...
  #########################
  #
  # Federal Carbon Tax with OBA for NB
  #
  market = 152
  ...
  for area in areas, pcov in PCovs, poll in polls
    @finite_math EIBase[IronSteel,poll,pcov,area] = 
      sum(PolTotRef[IronSteel,poll,pcov,area,year]*PolConv[poll]*
        ECoverage[IronSteel,poll,pcov,area,YrFinal] for year in Yr(2020)) /
          sum(DriverBaseline[IronSteel,area,year] for year in Yr(2020))
  end
  ...
  #
  for year in years, area in areas, ecc in eccs
    OBAFraction[ecc,area,year] = 0.99
  end
  ...
  # OBACogen[year] = OBAElectric[plant,fuelep,year]
