#
# CarbonTax_OBA_Fed_170.jl - Federal Carbon Tax with OBA for all PTs
#

using SmallModel

module CarbonTax_OBA_Fed_170
...
function ElecPolicy(db)
  ...
  (; FBuyFr,GoalPolSw,GratSw,ISaleSw,OBAFraction,OffMktFr) = data
  (; OffNew,PBnkSw,PCost,PCovMarket) = data
  (; PolConv,PolCovRef,PollMarket,xPolTot) = data
  (; UnArea,UnCogen,UnCoverage) = data
  (; UnF1,UnOffsets,UnOnLine) = data
  ...
  areas = Select(Area,["MB","PE","YT","NT","NU"])
  for year in years, area in areas
...
function PolicyControl(db)
  @info "CarbonTax_OBA_Fed_170.jl - PolicyControl"
  ElecPolicy(db)
end
