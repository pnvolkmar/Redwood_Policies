...
# CarbonTax_BC.jl
#
# This jl simulates the BC Carbon tax. The Tax begins in 2008 as nominal $10/tonne CO2e., 
# ramping up $5 per year until it reaches $30/tonne in 2012. 
# In Sep 2017, BC announced that on April 1st, 2018, the Carbon tax will go up $5 
# per year until it reaches $50/tonne in 2021. In terms of modelling, the 
# tax is inputted as a fuel tax in the history, and the forecast portion is covered by this jl.
# (J. St-Laurent O'Connor, 2017/09/14)
#
...
function ElecPolicy(db)
  ...
  for year in years, pcov in pcovs, poll in polls, ecc in eccs
    ECoverage[ecc,poll,pcov,Select(Area,"BC"),year] = 0.245
  end
  ...
  #########################
  # 
  # Carbon Tax - Starting at $30/tonne in first forecast year. Starting in 2018,it ramps up
  # $5/year until it reaches $50 in 2021. The price is flat beyond that. All $ values are nominal.
  # Carbon Prices are in nominal CN$/tonne
  #
  CN = Select(Nation,"CN")
  US = Select(Nation,"US")
  xETAPr[market,Yr(2016)] = 30.00/xExchangeRateNation[CN,Yr(2016)]/xInflationNation[US,Yr(2016)]
  xETAPr[market,Yr(2017)] = 30.00/xExchangeRateNation[CN,Yr(2017)]/xInflationNation[US,Yr(2017)]
  xETAPr[market,Yr(2018)] = 35.00/xExchangeRateNation[CN,Yr(2018)]/xInflationNation[US,Yr(2018)]
  xETAPr[market,Yr(2019)] = 40.00/xExchangeRateNation[CN,Yr(2019)]/xInflationNation[US,Yr(2019)]
  xETAPr[market,Yr(2020)] = 45.00/xExchangeRateNation[CN,Yr(2020)]/xInflationNation[US,Yr(2020)]
  xETAPr[market,Yr(2021)] = 50.00/xExchangeRateNation[CN,Yr(2021)]/xInflationNation[US,Yr(2021)]
  years = collect(Yr(2022):YrFinal)
  for year in years
    xETAPr[market,year] =  50.00/xExchangeRateNation[CN,year]/xInflationNation[US,year]
  end
...
function PolicyControl(db)
  @info"CarbonTax_BC.jl - PolicyControl"
  ElecPolicy(db)
end
